...
class Envio(models.Model):
    tipo = models.CharField(max_length=15, choices=[('Recojo', 'Recojo'), ('Envío', 'Envío')], default='Envío')
    remitente = models.ForeignKey(Usuario, related_name='remitente', on_delete=models.CASCADE)  
    ...
    estado = models.CharField(max_length=15, default='Pendiente', choices=[('Pendiente', 'Pendiente'), 
                                                                            ('En Ruta', 'En Ruta'), 
                                                                            ('Entregado', 'Entregado'), 
                                                                            ('Rechazado', 'Rechazado'), 
                                                                            ('Cancelado', 'Cancelado')])
    ...
    tipo_pago = models.CharField(max_length=15, choices=[('Origen', 'Origen'), ('Destino', 'Destino')])
    mensajero = models.ForeignKey(Usuario, related_name='mensajero', on_delete=models.CASCADE, null=True, blank=True)  # Relación con el mensajero
...
class Entrega(models.Model):
    ...
    pagado = models.BooleanField(default=False)  # Indica si ya se pagó o falta pagar
...
class HistorialEnvio(models.Model):
    envio = models.ForeignKey(Envio, on_delete=models.CASCADE)  # Relación con Envio
    tipo_evento = models.CharField(max_length=20, choices=[('Creado', 'Creado'), ('Asignado', 'Asignado'), 
                                                          ('Recogido', 'Recogido'), ('Entregado', 'Entregado'), 
                                                          ('Incidente', 'Incidente'), ('Cancelado', 'Cancelado')])
...
class Incidente(models.Model):
    envio = models.ForeignKey(Envio, on_delete=models.CASCADE)  # Relación con Envio
    tipo = models.CharField(max_length=20, choices=[('Retraso', 'Retraso'), ('Daño', 'Daño'), 
                                                    ('Pérdida', 'Pérdida'), ('Otro', 'Otro')])
